...
public class Util {
  ...
  public static PackageModel getPackageModelFromJar(File file)
    ...
    try (JarFile jarFile = new JarFile(file)) {
      ...
      try (InputStream stream = jarFile.getInputStream(entry)) {
        return JsonConverter.PACKAGE_MODEL_SERIALIZER.fromString(
            readAll(stream), PackageModel.class);
      }
    }
  }
   ...
   */
  public static FileCollection deduplicatedFileCollection(
      Project project, FileCollection... collections) {
   ...
   * @param required If {@code true}, this method will abort the build if no authorization is
   *     configured
  ...
  public static HttpHeaderCredentials getPublishCredentials(
    ...
    HttpHeaderCredentials publishCredentials =
        project.getObjects().newInstance(HttpHeaderCredentials.class);
    ...
    if (bearerToken != null) {
      ...
      FlintPluginProperty<String> publishTokenProperty =
          FlintPluginProperties.DISTRIBUTOR_PUBLISH_TOKEN;
      String publishToken =
          required
